...
simple_mean <- function(x){
  if(!is.numeric(x)){
    #construct an error
    categorical_column <- structure(
      class = c("categorical_column","error", "condition"),
      list(message = "Not sure what to do with categorical column",
           call = sys.call(),
           # and include the offending parameter in the error object
           x=x))
    # raise the error
    stop(categorical_column)
  }
...
.footnote[
_In the QandA I ask if we can convert this using `rlang::abort` and move away from `structure` and base R_
]
