...
knitr::include_graphics('metaprogramming-meme.jpeg')
```
...
"R code can be described as expressions, which can be drawn as tree."
...
> Complex expressions are built out of simpler expressions: `1 + 2` is an expression that uses `+` to combine 1 and 2, while the expression `c(10, 20, 30)` uses the function `c` to create a vector out of the values 10, 20, 30. Expressions are often drawn as trees.
...
With this knowledge, you can win debates.
...
+ Chapter 20: More about evaluatin (unquoting) captured code
