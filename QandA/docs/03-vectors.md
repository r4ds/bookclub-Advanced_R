...
```r
(an_array <- array(1:12, c(2, 3))) # this actually creates a class of matrix
```
...
```r
(one_d_array<-array(1:12, c(2, 3, 1)))
```
...
Let us do some testing with subsetting starting with the two matrices, one created using `matrix()` and the other using `array()`:
...
The subsetting for single entries also seems to work with the 2x2 array (with 3rd dimension = 1):
...
However, problems arise when we try to subset columns and rows. objects of class array require a different kind of subseting:
...
```
Error in one_d_array[, 1] : incorrect number of dimensions
```
...
To subset for this array class, we need to first access the specific 2x3 matrix, for example:
...
Look at this quick check:


```r
one_d_array == a_matrix
```

```
Error in one_d_array == a_matrix : non-conformable arrays
```


```r
one_d_array[,,1] == a_matrix
```
