...
`computeRestarts()` lists all available "default" restart functions, I believe and it looks like So, `computeRestarts()` is able to find `abort` even without `{rlang}` attached, then it surely would be able to find some function from the base packages if one existed. But it does not, so that leads me to believe that none exists. So you do always have to specify a function name for `invokeRestart()`
