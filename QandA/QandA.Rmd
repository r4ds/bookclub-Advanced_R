...
```{r}
df__non_syntactic_name %>% select("if")
df__non_syntactic_name %>% select(`if`)
```
...
When we assign the new value as an integer there is no intermediate step, this probably means `c(1,2, NA)` is the intermediate step 
```{r}
x <- c(1L, 2L, 3L)
obj_addr(x)
tracemem(x)
...
:::question
The `unwhich()` function in the book specifies the length of the vector we want of `FALSE` statements, why is this preferable to creating a vector of the length of the input itself? 
:::
