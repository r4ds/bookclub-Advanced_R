...
\begin{verbatim}
## █ [1:0x7fc837e779a0] <list> 
## └─[2:0x7fc837b178c0] <dbl>
\end{verbatim}
...
We can use the package \texttt{cyclocomp} to look at the cyclomatic complexity of our functions. Looking at each function from the Chapter 5 presentation:
...
We see that the \texttt{for} loop was our most complex function and \texttt{while} had a complexity of 3. The rest of our functions had a complexity of one. As \href{https://engineering-shiny.org/successfulshinyapp.html}{Colin Fay states} Splitting a complex function into smaller functions is not a magic solution because (A) the global complexity of the app is not lowered by splitting things into pieces (just local complexity), and (B) the deeper the call stack, the harder it can be to debug.
...
\KeywordTok{h03}\NormalTok{(}\KeywordTok{double}\NormalTok{(}\DecValTok{20}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\section*{6.5.3 Missing arguments}\label{missing-arguments}}
\addcontentsline{toc}{section}{6.5.3 Missing arguments}

Comparing the default sample function:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sample <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{ (x, size, }\DataTypeTok{replace =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{prob =} \OtherTok{NULL}\NormalTok{) }
\NormalTok{\{}
    \ControlFlowTok{if}\NormalTok{ (}\KeywordTok{length}\NormalTok{(x) }\OperatorTok{==}\StringTok{ }\NormalTok{1L }\OperatorTok{&&}\StringTok{ }\KeywordTok{is.numeric}\NormalTok{(x) }\OperatorTok{&&}\StringTok{ }\KeywordTok{is.finite}\NormalTok{(x) }\OperatorTok{&&}\StringTok{ }\NormalTok{x }\OperatorTok{>=}\StringTok{ }
\StringTok{        }\DecValTok{1}\NormalTok{) \{}
        \ControlFlowTok{if}\NormalTok{ (}\KeywordTok{missing}\NormalTok{(size))}
          \CommentTok{# if you don't supply the sample size}
          \CommentTok{# you will just get random samples}
          \CommentTok{# the length of the supplied vector}
\NormalTok{            size <-}\StringTok{ }\NormalTok{x}
        \KeywordTok{sample.int}\NormalTok{(x, size, replace, prob)}
\NormalTok{    \}}
    \ControlFlowTok{else}\NormalTok{ \{}
        \ControlFlowTok{if}\NormalTok{ (}\KeywordTok{missing}\NormalTok{(size)) }
\NormalTok{            size <-}\StringTok{ }\KeywordTok{length}\NormalTok{(x)}
\NormalTok{        x[}\KeywordTok{sample.int}\NormalTok{(}\KeywordTok{length}\NormalTok{(x), size, replace, prob)]}
\NormalTok{    \}}
\NormalTok{\}}
...
\begin{verbatim}
##  [1]  9 10  4  3  2  1  7  8  5  6
\end{verbatim}

To using Hadley's recommended \texttt{NULL} version:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{my_sample_null <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(x, }\DataTypeTok{size =} \OtherTok{NULL}\NormalTok{, }\DataTypeTok{replace =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{prob =} \OtherTok{NULL}\NormalTok{) \{}
...
\KeywordTok{my_sample_null}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\DecValTok{10}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
##  [1]  3  9  7  8  4  6 10  2  1  5
\end{verbatim}

The second function is obviously more concise, but why is Hadley recommending we we steer clear of \texttt{missing()}? Is this just for code read-ability or is there something inherent in the behavior of \texttt{missing} we need to consider that we don't when using \texttt{NULL}?
...
``(See also \texttt{rlang::list2()} to support splicing and to silently ignore trailing commas\ldots{}'' Can we come up with a simple use case for \texttt{list2} here?
...
``Always set \texttt{add\ =\ TRUE} when using \texttt{on.exit()} If you don't, each call to \texttt{on.exit()} will overwrite the previous exit handler.'' Can we come up with an example for not using \texttt{add\ =\ TRUE} and how it results in unwanted behavior?
