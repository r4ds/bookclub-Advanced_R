...
\begin{verbatim}
## █ [1:0x7f879236ae60] <list> 
## └─[2:0x7f879319dc40] <dbl>
\end{verbatim}
...
\begin{verbatim}
##  [1] 221 291 264 276 292 265 213 202 233 203
\end{verbatim}
...
\texttt{list2} is most helpful when we need to force environment variables with ata variables. We can see this by crting a function that takes a variable number of arguments:
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{body}\NormalTok{(capture.output)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\KeywordTok{identical}\NormalTok{(}
  \KeywordTok{capture.output}\NormalTok{(}\KeywordTok{cat}\NormalTok{(}\StringTok{"a"}\NormalTok{, }\StringTok{"b"}\NormalTok{, }\StringTok{"c"}\NormalTok{, }\DataTypeTok{sep =} \StringTok{"}\CharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)),}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{capture_output <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(..., }\DataTypeTok{file =} \OtherTok{NULL}\NormalTok{, append, }\DataTypeTok{type =} \StringTok{"output"}\NormalTok{, }\DataTypeTok{split =} \OtherTok{FALSE}\NormalTok{) \{}
  \CommentTok{# create a list from the arguments}
  \CommentTok{# but how does c("a", "b", "c") become the args?}
\NormalTok{  args <-}\StringTok{ }\KeywordTok{substitute}\NormalTok{(}\KeywordTok{list}\NormalTok{(...))[}\OperatorTok{-}\NormalTok{1L]}
  \CommentTok{# get type based on table of canditate values}
  \CommentTok{# but this errors when I try to just run }
  \CommentTok{# type = "output" ?}
\NormalTok{  type <-}\StringTok{ }\KeywordTok{match.arg}\NormalTok{(type)}
  \CommentTok{# Error in match.arg(type) : 'arg' must be of length 1}
\NormalTok{  rval <-}\StringTok{ }\OtherTok{NULL}
\NormalTok{  closeit <-}\StringTok{ }\OtherTok{TRUE}
  \CommentTok{# we can save the output to a textConnection if no file is specified}
  \CommentTok{# but what is a textConnection?}
  \CommentTok{# otherwise the output is saved to a file (I think?)}
  \ControlFlowTok{if}\NormalTok{ (}\KeywordTok{is.null}\NormalTok{(file)) }
    \CommentTok{# what is a text connection?}
    \CommentTok{# I know this returns a character vector but what is this function}
\NormalTok{    file <-}\StringTok{ }\KeywordTok{textConnection}\NormalTok{(}\StringTok{"rval"}\NormalTok{, }\StringTok{"w"}\NormalTok{, }\DataTypeTok{local =} \OtherTok{TRUE}\NormalTok{)}
  \ControlFlowTok{else} \ControlFlowTok{if}\NormalTok{ (}\KeywordTok{is.character}\NormalTok{(file)) }
    \CommentTok{# what is a character file? What's happening here?}
\NormalTok{    file <-}\StringTok{ }\KeywordTok{file}\NormalTok{(file, }\ControlFlowTok{if}\NormalTok{ (append)  }\StringTok{"a"} \ControlFlowTok{else} \StringTok{"w"}\NormalTok{)}
  \CommentTok{# this is similar to the character file but what is the difference?}
  \CommentTok{# what does it mean to inherit "connection"}
  \ControlFlowTok{else} \ControlFlowTok{if}\NormalTok{ (}\KeywordTok{inherits}\NormalTok{(file, }\StringTok{"connection"}\NormalTok{)) \{}
    \ControlFlowTok{if}\NormalTok{ (}\OperatorTok{!}\KeywordTok{isOpen}\NormalTok{(file))}
      \KeywordTok{open}\NormalTok{(file, }\ControlFlowTok{if}\NormalTok{ (append)  }\StringTok{"a"} \ControlFlowTok{else} \StringTok{"w"}\NormalTok{)}
    \ControlFlowTok{else}\NormalTok{ closeit <-}\StringTok{ }\OtherTok{FALSE}
\NormalTok{  \}}
  \CommentTok{# what is sink?}
  \CommentTok{# is this what happened when you create a bad file extension to save your output?}
  ...
  \CommentTok{# we're using on.exit here! }
  \CommentTok{# ...but what is it doing exactly?}
...
\NormalTok{  \})}
  \CommentTok{# this is some environment term}
  \CommentTok{# maybe we can talk about this next week?}
\NormalTok{  pf <-}\StringTok{ }\KeywordTok{parent.frame}\NormalTok{()}
  \CommentTok{# print value AND if visible or not }
\NormalTok{  evalVis <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(expr) }\KeywordTok{withVisible}\NormalTok{(}\KeywordTok{eval}\NormalTok{(expr, pf))}
  \CommentTok{# THIS IS THE ACTUAL CAPTURE FUNCTION MEAT AND POTATOES!}
  \ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \KeywordTok{seq_along}\NormalTok{(args)) \{}
    \CommentTok{# I assume this should be for each "a" "b" "c"}
    \CommentTok{# but I'm struggling to go one without it and being able to step through}
\NormalTok{    expr <-}\StringTok{ }\NormalTok{args[[i]]}
\NormalTok{    tmp <-}\StringTok{ }\ControlFlowTok{switch}\NormalTok{(}\KeywordTok{mode}\NormalTok{(expr), }\DataTypeTok{expression =} \KeywordTok{lapply}\NormalTok{(expr, evalVis), }
                  \DataTypeTok{call =}\NormalTok{ , }\DataTypeTok{name =} \KeywordTok{list}\NormalTok{(}\KeywordTok{evalVis}\NormalTok{(expr)), }\KeywordTok{stop}\NormalTok{(}\StringTok{"bad argument"}\NormalTok{))}
