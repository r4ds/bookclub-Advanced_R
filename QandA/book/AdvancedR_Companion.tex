...
\begin{verbatim}
## █ [1:0x7f976cae7170] <list> 
## └─[2:0x7f976ad6d878] <dbl>
\end{verbatim}
...
\begin{verbatim}
##  [1] 217 247 295 257 279 262 201 246 285 255
\end{verbatim}
...
We can use those two functions like we did getting and setting the work directory inside another function - using the old value within \texttt{on.exit} to reset \texttt{my\_env\$a} outside of the function

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{cleanup}\NormalTok{(value) \{}
\NormalTok{  my_env_a <-}\StringTok{ }\KeywordTok{get_a}\NormalTok{()}
  \KeywordTok{set_a}\NormalTok{(value) }
  \CommentTok{# now we can do stuff with the new my_env$a value}
  \KeywordTok{on.exit}\NormalTok{(}\KeywordTok{set_a}\NormalTok{(my_env_a), }\DataTypeTok{add=}\OtherTok{TRUE}\NormalTok{)}
\NormalTok{\} }
\end{Highlighting}
\end{Shaded}
