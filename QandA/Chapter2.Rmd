...
```{r}
read.csv(file = "docs/example2223.csv",check.names = TRUE)
```

Import using non-adjusted column names
```{r}
read.csv(file = "docs/example2223.csv", check.names = FALSE)
```

Import using the tidyverse where names are not adjusted
```{r}
df_non_syntactic_name  <- read_csv(file = "docs/example2223.csv")
```
