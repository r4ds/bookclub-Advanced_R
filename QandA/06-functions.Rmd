...
:::question
R looks for values when the function is run, not when the function is created - does anyone know if this is the case for Shiny modules?
:::
...
h05(ls())
```
I think this ties exercise 6.5.4.4
...
To using Hadley's reccomended `NULL` version:
...
The second function is obviously more concise, but why is Hadley reccomending we we steer clear of `missing()`? Is this just for code read-ability or is there something inherent in the behvior of `missing` we need to consider that we don't when using `NULL`?
:::
...
:::question
I know this isn't exactly needed to answer the question but how do we access a function that has methods? For instance - here I want to dig into the `hist` function but 
...
I know we have a chapter on tidyeval coming up, but can we come up with a simple use case for `list2` here?
:::
...
:::question
[This question is flagged as "started" let's try to complete it!](https://github.com/Tazinho/Advanced-R-Solutions/blob/5043d9b06c7469a010c568ecb85e12bedca75207/2-06-Functions.Rmd#L350) Hadley comments in the repo: "I think I'm more intersted in supplying a path vs. a logical value here".
:::
...
:::question
Can we go over the translation for 
