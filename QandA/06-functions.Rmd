...
```{r, eval=FALSE}
e1 <- rlang::env(f1 = function(x) (x - min(x)) / (max(x) - min(x))) 
e2 <- rlang::env(f2 = function(x) (x - min(x)) / (max(x) - min(x)))
```
...
`in_dir` gives you the flexibility to change the path to wherever you want (maybe a parent-level folder) whereas `source(chdir=TRUE)` changes the path to "where that file lives specifically". 
